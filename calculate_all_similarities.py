...
Pipeline stages applied per variant:
    ...
    [augment]   --shuffle (negative control)
...
def run_variant(
    ...
    Returns (success, skip_reason).  skip_reason is None on success or error.
    """
    ...
    if result.returncode != 0:
        return False, None  # None = error (not a skip)
...
def main() -> None:
    ...
    def _run(variant: dict) -> tuple[str, bool, str | None]:
        stem = variant_stem(variant)
        print(f"  → [{stem}] {variant['description']}")
        ok, reason = run_variant(templates, database, output_dir, variant, args.verbose, args.dry_run)
        if args.dry_run:
            cmd = [
                sys.executable,
                str(Path(__file__).parent / "smiles_similarity_kernels.py"),
    ...
    if jobs > 1:
        with ThreadPoolExecutor(max_workers=jobs) as pool:
            ...
            for fut in as_completed(futures):
                suffix, ok, reason = fut.result()
                results[suffix] = (ok, reason)
    else:
        for variant in variants_to_run:
            suffix, ok, reason = _run(variant)
            results[suffix] = (ok, reason)
    ...
    for v in variants_to_run:
        ...
        if reason is not None:
            ...
            print(f"  OK    {stem:45s} ({label})")
        else:
            failed_count += 1
            print(f"  FAIL  {stem:45s}")
